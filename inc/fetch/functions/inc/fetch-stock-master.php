...
if ($result === false) :
    ...
    if (file_exists(IQ_RETAIL_PATH . 'inc/fetch/files/stock-master.json')) :
        unlink(IQ_RETAIL_PATH . 'inc/fetch/files/stock-master.json');
    endif;

    // write result to file
    file_put_contents(IQ_RETAIL_PATH . 'inc/fetch/files/stock-master.json', $result);

    // logging
    $time_now = strtotime('now');

    if ($is_manual == true) :
        file_put_contents(IQ_RETAIL_PATH . 'inc/fetch/logs/manual-sync/stock-fetch.log', date('j F, Y @ h:i:s', $time_now) . ' - IQ stock fetch successful.' . PHP_EOL, FILE_APPEND);
    else :
        file_put_contents(IQ_RETAIL_PATH . 'inc/fetch/logs/auto-sync/stock-fetch.log', date('j F, Y @ h:i:s', $time_now) . ' - IQ stock fetch successful.' . PHP_EOL, FILE_APPEND);
    endif;
